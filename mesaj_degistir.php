...
if ($kip == 'mesaj')
{
	$vtsorgu = "SELECT id,yazan,mesaj_baslik,mesaj_icerik,bbcode_kullan,hangi_forumdan,ust_konu,kilitli,ifade
...
if ($kip == 'cevap')
{
	$vtsorgu = "SELECT id,cevap_yazan,cevap_baslik,cevap_icerik,bbcode_kullan,hangi_forumdan,hangi_basliktan,ifade
...
if ($mesaj_sahibi['resim'])
	$onizleme_resim = '<img src="'.$mesaj_sahibi['resim'].'" alt="Kulanıcı Resmi">';
...
if ( ($kip == 'mesaj') AND (($kullanici_kim['yetki'] == 1) OR ($kullanici_kim['yetki'] == 2)) )
{
	...
	if ( (isset($_POST['ust_konu'])) AND $_POST['ust_konu'] == 1)
		$form_ozellik .= ' checked="checked">Mesajı üst konu yap</label>';
	...
	else
	{
		...
		else $form_ozellik .= '>Mesajı üst konu yap</label>';
	}
}
...
$form_bilgi1 = '<form action="bilesenler/mesaj_degistir_yap.php" method="post" onsubmit="return '.$phpkf_duzenleyici.' denetle_duzenleyici()" name="form1">
...
$form_bilgi2 = '<form action="mesaj_degistir.php#onizleme" method="post" name="form2" onsubmit="return '.$phpkf_duzenleyici.' onizle_duzenleyici(), denetle_duzenleyici()">
<input type="hidden" name="fno" value="'.$fno.'">
<input type="hidden" name="mesaj_degisti_mi" value="form_dolu">
<input type="hidden" name="kip" value="'.$kip.'">
<input type="hidden" name="mesaj_no" value="'.$mesaj_no.'">
