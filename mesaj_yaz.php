...
if ($forum_satir['alt_forum'] != '0')
{
	...
	$alt_forum_baslik = '<a href="forum.php?f='.$forum_satir['id'].'">'.$forum_satir['forum_baslik'].'</a>';
}

else
{
	$ust_forum_baslik = '<a href="forum.php?f='.$forum_satir['id'].'">'.$forum_satir['forum_baslik'].'</a>';
	$alt_forum_baslik = '';
}
...
if ($kip == 'cevapla')
{
	$sayfa_baslik = '<a href="konu.php?k='.$mesaj_no.'&amp;fs='.$fsayfa.'&amp;ks='.$sayfa.'">'.$kilit_satir['mesaj_baslik'].'</a>';
}

else $sayfa_baslik = 'Yeni Konu Oluştur';
...
//	KULLANICI BİLGİLERİ ÇEKİLİYOR	//

$_COOKIE['kullanici_kimlik'] = $vt->real_escape_string($_COOKIE['kullanici_kimlik']);

$vtsorgu = "SELECT id,yetki,kullanici_adi,gercek_ad,resim,katilim_tarihi,mesaj_sayisi,sehir_goster,sehir,web,imza,ozel_ad
...
if ($ayarlar['bbcode'] == 1)
{
	...
	if ( (isset($_GET['alinti'])) OR (!empty($_POST['alinti'])) )
...
$tekli1[] = array('{YAZI_YAZAN}' => 'Başlık',
'{YAZI_YAZAN}' => 'Yazan',
'{YAZI_TARIH}' => 'Tarih',
...
$tekli1[] = array('{YAZI_YAZAN}' => 'Konu Başlığı',
...
// phpKF Düzenleyici
if ( ($ayarlar['duzenleyici'] == '') OR ($ayarlar['duzenleyici'] == 'varsayilan') )
$phpkf_duzenleyici = "yolla('mesaj_icerik_div','mesaj_icerik','yolla','cevir'),";
else $phpkf_duzenleyici = '';


$form_bilgi1 = '<form action="bilesenler/mesaj_yaz_yap.php" method="post" onsubmit="return '.$phpkf_duzenleyici.' denetle_duzenleyici()" name="form1">
...
$form_bilgi2 = '<form action="mesaj_yaz.php#onizleme" method="post" name="form2" onsubmit="return '.$phpkf_duzenleyici.' onizle_duzenleyici(), denetle_duzenleyici()">
<input type="hidden" name="fno" value="'.$fno.'">
<input type="hidden" name="kayit_yapildi_mi" value="form_dolu">
<input type="hidden" name="kip" value="'.$kip.'">
<input type="hidden" name="mesaj_no" value="'.$mesaj_no.'">
