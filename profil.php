...
if ( (isset($_GET['u'])) AND ($_GET['u'] != '') )
{
	...
	$vtsorgu = "SELECT id,kullanici_adi,gercek_ad,dogum_tarihi_goster,dogum_tarihi,sehir_goster,sehir,posta_goster,posta,web,katilim_tarihi,mesaj_sayisi,resim,imza,yetki,engelle,gizli,son_hareket,kul_etkin,hangi_sayfada,sayfano,grupid,icq,msn,yahoo,aim,skype,ozel_ad,cinsiyet,hakkinda,yrm_sayi,yrm_yapilan
...
else
{
	...
	$vtsorgu = "SELECT id,kullanici_adi,gercek_ad,dogum_tarihi_goster,dogum_tarihi,sehir_goster,sehir,posta_goster,posta,web,katilim_tarihi,mesaj_sayisi,resim,imza,yetki,engelle,gizli,son_hareket,kul_etkin,hangi_sayfada,sayfano,grupid,icq,msn,yahoo,aim,skype,ozel_ad,okunmamis_oi,cinsiyet,hakkinda,yrm_sayi,yrm_yapilan
	FROM $tablo_kullanicilar WHERE kullanici_kimlik='$_COOKIE[kullanici_kimlik]' LIMIT 1";

	$vtsonuc = $vt->query($vtsorgu) or die ($vt->hata_ver());
	$satir = $vt->fetch_array($vtsonuc) or die ($vt->hata_ver());
...
if ($satir['web']) $uye_web = '<a href="'.$satir['web'].'" target="_blank" rel="nofollow">'.str_replace('http://', '', $satir['web']).'</a>';
else $uye_web = '';
...
if ($satir['aim'] != '') $uye_aim = '<a href="'.$satir['aim'].'" target="_blank">Tıklayın</a>';
else $uye_aim = '';
...
if ($satir['yahoo'] != '') $uye_yahoo = '<a href="http://members.yahoo.com/interests?.oc=t&amp;.kw='.$satir['yahoo'].'&amp;.sb=1" target="_blank">'.$satir['yahoo'].'</a>';
else $uye_yahoo = '';



if ($satir['skype'] != '') $uye_skype = '<a href="'.$satir['skype'].'" target="_blank">Tıklayın</a>';
else $uye_skype = '';



if ($satir['resim'] != '') $uye_resim = '<img src="'.$satir['resim'].'" alt="Kullanıcı Resmi" title="Kullanıcı Resmi">';
elseif ($ayarlar['kul_resim'] != '') $uye_resim = '<img src="'.$ayarlar['kul_resim'].'" alt="Varsayılan Kullanıcı Resmi">';
else $uye_resim = '';
